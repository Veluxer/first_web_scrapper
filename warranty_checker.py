...
def graphics():
   #tkinter window open and display text
   window.geometry('600x800')
   window.title("Stagiaire")
   lbl = tk.Label(window, text="\bAppuyer sur le bouton si vous avez\nmis les fichiers .txt a examiner dans\nvotre dossier content.\nnormalement il est sur votre bureau.", font=("Arial", 25))
   lbl.grid(column=0, row=0)
   b = tk.Button(window, text="Je suis pret", command=application)
   b.grid(column=1, row=3)
   window.mainloop()
   return window

def error():
   #in case of error, even if the script has no problem it will pass here before exiting
    def error_button():
        exit()
    if i == 1:
        exit()
    ...
    window.title("Error")
    b = tk.Button(window, text="OK", command=error_button)
    b.pack()
    window.mainloop()
